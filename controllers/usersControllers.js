...
const updateUserAvatar = async (req, res) => {
    const updatedUser = await usersServices.updateUserAvatar(req.user.id, req.file);
...
const followUser = async (req, res) => {
    const response = await usersServices.follow(req.user.id, req.params.followerId);

    res.status(200).json(response);
};

const unfollowUser = async (req, res) => {
    const response = await usersServices.unfollow(req.user.id, req.params.followerId);
